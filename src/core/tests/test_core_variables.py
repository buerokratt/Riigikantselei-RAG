...
class TestCoreVariableViews(APITestCase):
    ...
    def test_changed_core_settings_returns_expected_value(self):
        ...
        response = self.client.post(
            self.list_url, data={'name': 'ELASTICSEARCH_URL', 'value': new_value}
        )
    ...
    def test_floats_and_integers_being_parsed_as_numbers_and_not_strings(self):
        float_value = 13.5
        response = self.client.post(
            self.list_url, data={'name': 'OPENAI_API_TIMEOUT', 'value': float_value}
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_unknown_keys_being_rejected_by_validation(self):
        response = self.client.post(
            self.list_url, data={'name': 'FAKE FAKE FAKE LEMON FAKE', 'value': 'Holy Handgrenade'}
        )
    ...
    def test_api_keys_and_secrets_being_censored(self):
        ...
        response = self.client.post(
            self.list_url, data={'name': 'OPENAI_API_KEY', 'value': key_value}
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_simple_detail_view_access(self):
        ...
        detail_url = reverse('core_settings-detail', kwargs={'pk': response_id})
        detail_response = self.client.get(detail_url)
        self.assertEqual(detail_response.status_code, status.HTTP_200_OK)
