...
from elasticsearch import Elasticsearch, NotFoundError, RequestError, AuthenticationException, ConnectionError as ElasticsearchConnectionError, ConnectionTimeout
...
def elastic_connection(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        # Important to set the ConnectionError to the bottom of the chain as it's one of the superclasses the other exceptions inherit.
            ...
            raise APIException(ELASTIC_UNKNOWN_ERROR_MESSAGE, code=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class ElasticCore:
    ...
    @elastic_connection
    def create_index(self, index_name: str, shards: int = 3, replicas: int = 1, settings: Optional[dict] = None):
        body = settings or {
    ...
    @elastic_connection
    def add_vector_mapping(self, index: str, field: str, body: Optional[dict] = None, dims: int = 1024):
        mapping = body or {
            "properties": {
                field: {
                    "type": "dense_vector",
    ...
    def add_vector(self, index: str, document_id: str, vector: List[float], field: str):
        return self.es.update(
            index=index,
            id=document_id,
            body={"doc": {field: vector}},
            refresh="wait_for"
        )

    @elastic_connection
    def search_vector(self, indices: str, vector: List[float], comparison_field: str, search_query: dict = MATCH_ALL_QUERY):
        query = {
            "query": {
                "script_score": {
                    **search_query,
